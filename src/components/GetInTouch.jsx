    ...
    <section id="contact" ref={ref} className="px-6 md:px-16 lg:px-24 py-20 md:py-28 bg-beige-light">
      <div className="max-w-6xl mx-auto">
        ...
        <div className="grid md:grid-cols-2 gap-8">
          ...
          <motion.div variants={fadeInUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'} transition={{ duration: 0.6, delay: 0.2 }} className="space-y-4">
            <a href={WHATSAPP_URL} target="_blank" rel="noopener noreferrer"
              ...
              <div><p className="font-bold">WhatsApp</p><p className="text-white/70 text-sm">{WHATSAPP_DISPLAY}</p></div>
            </a>

            <a href={`tel:${WHATSAPP_TEL}`} className="flex items-center gap-4 bg-brown-earth rounded-2xl p-5 text-white hover:shadow-xl transition-all duration-300 hover:-translate-y-0.5 group">
              <div className="w-12 h-12 rounded-full bg-white/20 flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform">
                <svg className="w-6 h-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" /></svg>
              </div>
              <div><p className="font-bold">Call Us</p><p className="text-white/70 text-sm">{WHATSAPP_DISPLAY}</p></div>
            </a>
            ...
            <a href={INSTAGRAM_URL} target="_blank" rel="noopener noreferrer"
              ...
              <div><p className="font-bold">Instagram</p><p className="text-white/70 text-sm">@{INSTAGRAM_HANDLE}</p></div>
            </a>
          </motion.div>
          ...
          <motion.div variants={fadeInUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'} transition={{ duration: 0.6, delay: 0.35 }} className="space-y-4">
            ...
            <a href={whatsappLink('Hi! I would like to visit the farm')} target="_blank" rel="noopener noreferrer"
        ...
        <motion.div variants={fadeInUp} initial="hidden" animate={isInView ? 'visible' : 'hidden'} transition={{ duration: 0.6, delay: 0.5 }}
          className="mt-10 bg-green-deep rounded-2xl p-6 md:p-8 flex flex-col md:flex-row items-center gap-6">
          <div className="flex-1 text-center md:text-left">
            <h3 className="text-xl font-bold text-white mb-1">Join Our Farm Family</h3>
            <p className="text-white/70 text-sm">Weekly harvest updates, seasonal recipes, farm stories & first access to fresh produce.</p>
          </div>
          <a href={whatsappLink('Hi! I want to join the Nimmu Naturals community')} target="_blank" rel="noopener noreferrer"
            className="shrink-0 inline-flex items-center gap-2 bg-white text-green-deep px-6 py-3 rounded-full font-bold hover:bg-beige transition-all duration-300 hover:shadow-lg text-sm">
